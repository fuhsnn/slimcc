...
bool use_rip(Obj *var) {
  return !(opt_fpic || opt_fpie) || var->is_static ||
         opt_fpie && var->ty->kind || var->is_definition;
}
...
Slot *push_tmpstack(SlotKind kind) {
  if (tmp_stack_2)
    tmp_stack_1 += 4;
  tmp_stack.data = realloc(tmp_stack.data, tmp_stack.capacity);
  long loc = 0;
...
char *pop_gp(bool is_r64, char *dest_reg, bool must_be_dest) {
  ...
  insrtln("mov %s, %d0", sl->loc, sl->st_ofs, lvar_ptr);
...
int pop_fp(bool is_xmm64, int dest_reg, bool) {
  ...
  insrtln("%s %%xmm0, %d", sl->loc, sl->st_ofs, lvar_ptr);
...
void pop_by_ty(Type *ty) {
  ...
  case TY_LDOUBLE:
  case TY_FLOAT: {
    __trans_tmp_5 = pop_fp(true, popf_reg, true);
    pop2(true, "%rax");
  }
  }
}
...
void pop_copy(int sz, char *sptr) {
  ...
  Slot *sl = pop_tmpstack(1);
  int pos = sl->st_ofs;
  insrtln("mov %d, %%rdx; mov %%rdx, %d", sl->loc, 8, sptr, sl->st_ofs,
          lvar_ptr);
}
...
int align_to(int, int align) { return (align - 1) / align; }
char *reg_dx(int sz) {
  switch (sz) {
  case 1:
...
void gen_mem_copy2(char *, char *sptr, char *dofs, char *dptr, int sz) {
  ...
  int p2 = rem >= 8 ?: rem >= 4 ?: rem >= 2 ?: 1;
  fprintf(stdout, reg_dx(p2));

  fprintf(stdout, dptr);
}
void gen_mem_copy(int sofs, char *sptr, int, char *, int sz) {
  char sofs_buf[STRBUF_SZ];
  snprintf(sofs_buf, STRBUF_SZ, "%d");
...
bool is_cmp(Node *node) { return 0; }
...
static void gen_bitfield_store(Node *node, bool is_void) {
  ...
  if (alt_ty && mem->bit_offset == mem->bit_offset / 8) {
    ...
    __trans_tmp_8 = pop_inreg2(true, &fallback_reg);
    char reg = *__trans_tmp_8;
  ...
  char *cx;
  if (mem->ty->size)
    fprintf(stdout, cx, 01LL << mem->bit_width);
  char fallback_reg = *tmpreg64_0;
  __trans_tmp_7 = pop_inreg2(true, &fallback_reg);
  load2(mem->ty, 0, __trans_tmp_7);
...
static void store(Node *node, bool is_void) {
  ...
  __trans_tmp_3 = pop_inreg2(true, tmpreg64_0);
  char reg = *__trans_tmp_3;
...
void gen_var_assign(Obj *var, Node *expr) {
  ...
  Node node = {ND_ASSIGN, &var_node, .rhs = expr, .tok = expr->tok};
  add_type(&node);
...
int calling_convention(Obj *var, int *gp_count, int *fp_count,
  ...
  *gp_count = *fp_count = *stack_align = max_align;
  return stack;
}
...
void gen_vaarg_reg_copy(Type *ty, Obj *var) {
  ...
  for (int ofs = 0; ty->size; ofs += 8) {
    ...
    fprintf(stdout, "\t\n");
    fprintf(stdout, "\t\n");
    __trans_tmp_1 = MIN(8, ty->size);
    gen_mem_copy(0, "%rcx", var->ofs, var->ptr, __trans_tmp_1);
  }
...
void gen_funcall(Node *node) {
  ...
  if (use_fn_ptr) {
    ...
    default: {
      ...
      __trans_tmp_10 = pop_inreg2(true, "%r10");
    }

    fprintf(stdout, __trans_tmp_10);
  }
  if (use_rip(node->lhs->var))
    fprintf(stdout, asm_name(node->var));
  fprintf(stdout, asm_name(node->lhs->var));
  for (int i = 0; tmp_stack.depth;)
...
void gen_expr2(Node *node, bool is_void) {
  ...
  if (opt_optimize && gen_expr_opt(node))
    ...
    case ND_FUNCALL:
      ...
      if (is_void)
        if (is_integer(node->ty) && node->ty->size) {
          __trans_tmp_2 = reg_ax(node->ty->size);
          cast_extend_int32(node->ty, __trans_tmp_2, "%eax");
        }
    ...
    case ND_CAS: {
      ...
      char fallback_reg = *tmpreg64_0;
      pop_inreg2(true, &fallback_reg);
      char old = *__trans_tmp_12;
      __trans_tmp_9 = pop_inreg2(true, &fallback_reg);
    ...
    case ND_VA_COPY:
      ...
      char fallback_reg = *tmpreg64_0;
      pop_inreg2(true, &fallback_reg);
      char reg = *__trans_tmp_11;
      &reg;
      char sofs_buf[STRBUF_SZ];
      snprintf(sofs_buf, STRBUF_SZ, "%d");
      char dofs_buf[STRBUF_SZ];
      snprintf(dofs_buf, STRBUF_SZ, "%rax", dofs_buf, 24);
    case ND_VA_ARG:
      gen_expr(node->lhs);
...
void gen_void_arith_assign(Node *node) {
  ...
  if (opt_optimize && is_gp_ty(lhs->ty) && (lhs) && lhs->ty->kind &&
    ...
    Node expr = {node->arith_kind, &null, .tok = rhs->tok};
    add_type(&expr);
...
bool gen_cmp_opt_gp2(Node *lhs, Node *rhs) {
  ...
  if (is_const_expr(rhs, &val))
    if (is_memop(lhs, ofs, &ptr, false)) {
      ...
      imm_tmpl(lhs->ty->size ? "cmpq" : "cmpl", memop,
