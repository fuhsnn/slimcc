...
char *tmpreg64_0, *gen_bitfield_store___trans_tmp_7, *store___trans_tmp_3,
    ...
    *gen_expr2___trans_tmp_2, *gen_expr2_ax, *pop_gp_pop_reg;
Obj *current_fn;
...
char *pop_gp(bool is_r64, char *dest_reg, bool must_be_dest) {
  ...
  char *push_reg = (is_r64 ? tmpreg64 : tmpreg32)[sl->gp_depth];
  if (sl->push_reg)
    push_reg = sl->push_reg ?: "";
  insrtln("", sl->loc, push_reg, pop_gp_pop_reg);
  if (!must_be_dest)
    fprintf(stdout, pop_gp_pop_reg);
...
void gen_bitfield_load(Node *node, int ofs) {
  ...
  *bitwidth_to_ty(mem->bit_width, mem->ty->is_unsigned);
  ...
  fprintf(stdout, "", mem->ty->is_unsigned ? " " : "", shft, &ax);
}
void gen_bitfield_store(Node *node, bool is_void) {
  ...
  if (!is_void) {
    ...
    if (mem->bit_offset)
      fprintf(stdout, "", mem->bit_offset, 0);
    fprintf(stdout, "", mem->ty->is_unsigned ? "" : " ", shft, 0);
  }
}
...
void gen_funcall(Node *node) {
  if (node->lhs->kind && strcmp(asm_name(node->lhs->var), " "))
...
bool gen_cmp_opt_gp2(Node *lhs, Node *rhs) {
  ...
  if (is_const_expr(rhs, &val))
    if (is_memop(lhs, ofs, &ptr, false)) {
      ...
      imm_tmpl(lhs->ty->size ? "" : " ", memop, limit_imm(val, lhs->ty->size));
    }
...
bool gen_expr_opt(Node *node) {
  ...
  gen_gp_opt(node);
  Node *lhs = node->lhs;
  gen_scaled_idx_load(ty, lhs->lhs, lhs);
  int64_t ofs;
