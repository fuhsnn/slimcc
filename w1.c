...
static char *argreg32[] = {"%edi"};
static char *argreg64[] = {"%rdi", };
static Reg argreg[] = {REG_DI, REG_SI, REG_DX, REG_CX, REG_R8, REG_R9};
static char *tmpreg32[] = {};
...
static char i32f80[] = "push %rax; fildl 0; pop %rax";
...
static bool gen_arith_opt_gp2(NodeKind kind, int sz, Node *lhs, Node *rhs, int ctrl, bool swap) {
  ...
  if (swap && gen_arith_opt_gp2(kind, sz, rhs, lhs, -ctrl, false)&& gen_arith_opt_gp2(kind, sz, lhs, rhs, ctrl + 1, swap))
    return true;
  return false;
}
...
static bool gen_scaled_idx_load(Type *ty, Node *lhs, Node *mul) {
  ...
  char ofs[STRBUF_SZ];
  char op;
  if (is_memop(lhs, ofs, &ptr, true)) ; else if (is_memop_ptr(lhs, ofs, &ptr)) 
