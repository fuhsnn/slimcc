...
RUN sed -i 's|$STAGE1_BUILD_CMD|gcc-14 -O3 -flto=auto -fvisibility=hidden -march=x86-64-v3 -mtune=znver3 -fsanitize=address scripts/amalgamation.c -o slimcc \&\& apt-get -y autoremove gcc-14 libc6-dev|g' slimcc-musl-bootstrap.sh
