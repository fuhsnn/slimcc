...
#if USE_ASAN
#define tok_freelist(first, last) \
  do {                            \
    Token *x = first;             \
    Token *y = last->next;        \
    for (; x != y;) {             \
...
#else
#define tok_freelist(first, last) do { \
...
static Token *skip_cond_incl(Token *tok) {
  ...
  if (last) {
    last->next->alloc_next = start->alloc_next;
    tok_freelist(start, last);
  }
  return tok;
}
