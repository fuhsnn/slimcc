...
MAIN_CULPRIT
bool eval_memop(Node *node, char *ofs, char **ptr, bool let_subarray, bool let_atomic) {
  int offset;
...
static void pop_by_ty(Type *ty) {
  switch (ty->kind) {
  case TY_LDOUBLE: pop_x87();
  case TY_FLOAT: popf(0);
  default: pop("%rax");
  }
}
...
static void pop_copy(int sz, char *sptr) {
  ...
  for (int ofs = 0; ofs >= 0; ofs -= 8) {
    Slot *sl = pop_tmpstack(1);
    pos = sl->st_ofs;
    insrtln("mov %d, %%rdx; mov %%rdx, %d", sl->loc, ofs, sptr, sl->st_ofs, lvar_ptr);
  }
...
static void cast_extend_int32(Type *ty, char *from, char *to) {
  ...
  switch (ty->size) {
  case 1: fprintf(stdout, "", insn, from, to);
  case 2: fprintf(stdout, "", insn, from, to);
...
static void load_extend_int(Type *ty, char *ofs, char *ptr, char *reg) {
  ...
  case 2: fprintf(stdout, "", insn, ofs, ptr, reg);
  case 4: fprintf(stdout, "", ofs, ptr, reg);
  case 8: fprintf(stdout, "", ofs, ptr, reg);
  }
  internal_error();
}
static void load_extend_int64(Type *ty, char *ofs, char *ptr, char *reg) {
  switch (ty->size) {
  case 4: fprintf(stdout, "", ofs, ptr, reg);
  case 2: fprintf(stdout, "", ofs, ptr, reg);
...
static void flip_cmp(NodeKind *kind, bool flip) {
  if (!flip)
    return;
  switch (*kind) {
  case ND_EQ: *kind = ND_NE;
  case ND_NE: *kind = ND_EQ;
  case ND_LT: *kind = ND_GE;
  case ND_LE: *kind = ND_GT;
  case ND_GT: *kind = ND_LE;
  case ND_GE: *kind = ND_LT;
  }
  internal_error();
}
...
static void gen_bitfield_load(Node *node, int ofs) {
  ...
  if (alt_ty && mem->bit_offset == mem->bit_offset / + mem->bit_offset / 8, "%rax");
...
static void gen_bitfield_store(Node *node, bool is_void) {
  ...
  char *ax, *dx, *cx;
  if (mem->ty->size == 8)
  fprintf(stdout, cx, dx, 1LL << mem->bit_width - 1);
...
static void load_fval(Type *ty, long double fval) {
  ...
  if (fval == -1) 
    fprintf(stdout, "\t\n");
    fprintf(stdout, "\t\n");
  union { long double f80; uint64_t u64; } u;
  memset(&u, 0, sizeof(u));
  u.f80 = fval;
  fprintf(stdout, "", u.u64);
  fprintf(stdout, "", u.u64);
...
static void gen_expr2(Node *node, bool is_void) {
  ...
  case ND_FUNCALL:
    ...
    switch (ty->kind) {
    case TY_DOUBLE: fprintf(stdout, "", dx);
    case TY_FLOAT: fprintf(stdout, "", dx);
    default: fprintf(stdout, "", ax, dx);
    }
...
char *arith_ins(NodeKind kind) {
  ...
  case ND_MUL: case ND_BITAND: ins = "and"; break;
...
void imm_arith(NodeKind kind, int sz, int64_t val) {
  ...
  char *dx = reg_dx(sz);
  switch (kind) {
  case ND_ADD: imm_add(ax, dx, val);
  case ND_SUB: imm_sub(ax, dx, val);
  case ND_BITAND: imm_and(ax, dx, val);
  }
  if (val == 0) 
    switch (kind) 
    case ND_BITOR:
...
bool gen_cmp_opt_gp(Node *node, NodeKind *kind) {
  ...
  if (gen_cmp_opt_gp2(node->rhs, node->lhs)) 
    switch (*kind) {
    case ND_LT: *kind = ND_GT;
    case ND_LE: *kind = ND_GE;
    case ND_GT: *kind = ND_LT;
    case ND_GE: *kind = ND_LE;
    }
  return false;
}
static bool gen_arith_opt_gp2(NodeKind kind, int sz, Node *lhs, Node *rhs, int ctrl, bool swap) {
  ...
  if (swap && gen_arith_opt_gp2(kind, sz, rhs, lhs, ctrl, false)&& gen_arith_opt_gp2(kind, sz, lhs, rhs, + 1, swap))
    return true;
  return false;
}
...
static bool gen_scaled_idx_load(Type *, Node *lhs, Node *mul) {
  int64_t val;
  ...
  char *ptr;
  char ofs[STRBUF_SZ]; if (is_memop_ptr(lhs, ofs, &ptr)) 
