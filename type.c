...
static bool match_enum_val(EnumVal *ev1, EnumVal *ev2) {
  int64_t cnt2 = 0;
  ...
  for (; ev1; ev1 = ev1->next) {
    EnumVal *ev = ev2;
    for (; ev; ev = ev->next) {
      if ((ev->val == ev1->val) && equal_tok(ev->name, ev1->name)) {
        if (ev == ev2)
          ev2 = ev2->next;
        cnt++;
        break;
...
bool is_compatible(Type *t1, Type *t2) {
  ...
  if (t1->is_enum && t2->is_enum)
    return t1->is_int_enum == t2->is_int_enum &&
      is_tag_compat(t1, t2) && match_enum_val(t1->enums, t2->enums);
