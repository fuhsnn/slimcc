...
## Building
 ...
 - For recent glibc-based Linux, `platform/linux-glibc-generic.c` should just work.
 - Default build finds builtin headers by relative path and is expected to be executed in source directory like `CC=~/slimcc/slimcc`. If you are installing/symlinking/packaging, please update `platform.c` with absolute paths.
 - `make slimcc-asan/test-asan` build and run tests with address sanitizer, recommended if you are hacking the source.
 - Optimization flags and custom allocators are quite impactful; try `make lto/lto-mi/lto-je` for a quick taste.
